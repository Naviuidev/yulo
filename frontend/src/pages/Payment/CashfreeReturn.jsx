...
export default function CashfreeReturn() {
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!orderRef) {
        setStatus('failed');
        setMessage('Missing order reference from Cashfree.');
        return;
      }

      try {
        const res = await paymentService.verifyCashfree(orderRef);
        if (cancelled) return;
    ...
    <>
      ...
      <div className="container py-5 text-center" style={{ maxWidth: 520 }}>
        <div className={`display-6 mb-3 ${status === 'paid' ? 'text-success' : 'text-muted'}`}>
          <i className={`bi ${status === 'paid' ? 'bi-check-circle' : status === 'pending' ? 'bi-hourglass-split' : 'bi-x-circle'}`} />
        </div>
        <h1 className="h4 mb-2">{status === 'paid' ? 'Payment Successful' : status === 'pending' ? 'Payment Pending' : 'Payment Failed'}</h1>

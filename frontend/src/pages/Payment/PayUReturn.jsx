...
export default function PayUReturn() {
  ...
  const orderRef =
    searchParams.get('order_id') || searchParams.get('txnid') || searchParams.get('orderId') || '';
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!orderRef) {
        setStatus('failed');
        setMessage('Missing order reference from PayU.');
        return;
      }

      try {
        const res = await paymentService.verifyPayU(orderRef);
        if (cancelled) return;

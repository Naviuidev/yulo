...
export default function RazorpayReturn() {
  const [searchParams] = useSearchParams();
  const navigate = useNavigate();
  const { refreshCart } = useCart();

  const yuloOrderId = searchParams.get('order_id') || '';
  const razorpayOrderId = searchParams.get('razorpay_order_id') || '';
  const razorpayPaymentId = searchParams.get('razorpay_payment_id') || '';
  const razorpaySignature = searchParams.get('razorpay_signature') || '';

  const [status, setStatus] = useState('checking'); // checking | paid | pending | failed
  const [orderId, setOrderId] = useState(yuloOrderId ? Number(yuloOrderId) || null : null);
  ...
  useEffect(() => {
    ...
    (async () => {
      if (!razorpayOrderId || !razorpayPaymentId || !razorpaySignature) {
        setStatus('failed');
        setMessage('Missing payment details from Razorpay. You can check the order under My Orders.');
        return;
      }
      ...
      try {
        ...
        setOrderId(data?.order_id ?? (yuloOrderId ? Number(yuloOrderId) : null));
  ...
  }, [razorpayOrderId, razorpayPaymentId, razorpaySignature, yuloOrderId, refreshCart]);
